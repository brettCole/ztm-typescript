...
// perform step 1 here
const smallSpaces = vendors.filter(el => el.size <= 10);
...
// perform step 2 here
const sizeExists = vendors.some(el => el.size === 13);
assert.equal(sizeExists, true);

// perform step 3 here
const updatedDuration = vendors.map(el => ({
  ...el,
  presentationDuration: el.presentationDuration + 3,
}));
